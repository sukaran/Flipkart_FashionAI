...
zip_loc = 'C:/Users/sgsox/Desktop/15 days/Placement 2020/FashionTrends/FashionTrends/Image_Data/Chictopia/Parsed/Clustered/Clustered.zip'
...
dataset['Pant'] = dataset.groupby(['Img', 'Date'])['Pant'].transform('sum').clip(upper=1.0)
dataset['Skirt'] = dataset.groupby(['Img', 'Date'])['Skirt'].transform('sum').clip(upper=1.0)
dataset['Dress'] = dataset.groupby(['Img', 'Date'])['Dress'].transform('sum').clip(upper=1.0)
dataset['Shirt'] = dataset.groupby(['Img', 'Date'])['Shirt'].transform('sum').clip(upper=1.0)
